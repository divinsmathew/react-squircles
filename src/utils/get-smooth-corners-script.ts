  ...
  class ReactSquirclePainter {
    ...
    paint(ctx, geom, properties) {
      const [nX, nY] = properties
        ...
        .replace(/ /g, "")
        .split(",");
      const backgroundColor = properties.get("--squircle-background-color").toString();
      ...
      let squircleCoordinates = this.squircle(
        width,
        height,
        parseFloat(nX, 10),
        parseFloat(nY, 10)
      ...
      squircleCoordinates.forEach((point, i) => {
        if (i === 0) {
      ...
      if (borderWidth || borderColor) {
        ...
        squircleCoordinates = this.squircle(
